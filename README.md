...
```bash
.venv/bin/python3 -m pytest chatmcd/ eval/ scripts/   # 68 tests
...
- [x] **A live status light.** The dot beside Marc's name is green when the model is answering, amber when it is queued or waking, and red when it is not responding, with the reason in its tooltip. `/api/health` answers from what the client last observed and falls back to a cached handshake, so an open tab polling every 45 seconds never touches a GPU. It also updates the instant a conversation succeeds or fails, because by then the visitor has better evidence than any poll. It reported a real outage correctly on its first live run.
